...
To have a **general overview of the ny_noaa dataset**, I use the `str` function to obtain a summary of the internal data structure of this dataframe.
...
### NOAA data wrangling
Next, I do some **data cleaning and wrangling** including:
* Set reasonable names using `janitor::clean_names` function
* Drop NA using `dumpster`
* Create separate variables for year, month, and day using `mutate`
* Ensure observations for temperature, precipitation, and snowfall are given in reasonable units using XX

```{r}

```

For snowfall, the most commonly observed values are XX
