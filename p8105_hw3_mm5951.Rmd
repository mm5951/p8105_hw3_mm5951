...
snowfall = noaa_df %>%
  ...
  ggplot(aes(x = year, y = mean(snow))) +
  geom_violin() +
  labs(title = "Snowfall Distribution over time (0-100 mm)",
       x = "Time (years)",
       y = "Snowfall (mm)",
       caption = "Exploratory analysis from the ny_noaa dataset")
...
As for the right panel, we observe that the average snowfall has oscillated between 31.50 mm and 32.00 mm each year, with wider variations in the last year.


```{r}
snowfall2 = noaa_df %>%
  filter(snow > 0, snow < 100) %>%
  ggplot(aes(x = year, y = sum(snow))) +
  geom_violin() +
  labs(title = "Snowfall Distribution over time (0-100 mm)",
       x = "Time (years)",
       y = "Snowfall (mm)",
